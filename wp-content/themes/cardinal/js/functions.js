...
//			jQuery('.blog-items:not(.carousel-items),article.type-portfolio,article.type-post,article.type-team,.spb_video_widget,.infocus-item,.full-width-detail,#activity-stream').fitVids({ ignore: '.no-fv'});
		...
		stickySidebars: function() {
			...
			var topMargin = 20,
				topOffset = jQuery('.sidebar').offset().top + topMargin;
							
			if (body.hasClass('sticky-header-enabled') && jQuery('.sticky-header').length > 0) {
				topMargin += jQuery('.sticky-header').height();
			}

			if (jQuery('#wpadminbar').length > 0) {
				var adminbarHeight = jQuery('#wpadminbar').height();
				topMargin += adminbarHeight;
				topOffset += adminbarHeight;
			}
			
			jQuery('.sidebar > .sidebar-widget-wrap').css('top', topMargin).affix({
				offset: {
				    top: topOffset,
				    bottom: function () {
		...
		pageTransitions: function() {
			jQuery('a').on('click', function(e) {
				...
				if (link.indexOf('#') === 0 && link.length > 1) {
					...
					return;			
				} else if (linkTarget === '_blank' || link.indexOf('?') >= 0 || link.indexOf('.jpg') >= 0 || link.indexOf('.png') >= 0 || link.indexOf('mailto') >= 0 || e.ctrlKey || e.metaKey || link.indexOf('javascript') === 0) {
					return e;
				} else {					
